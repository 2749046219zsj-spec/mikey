/**
 * 多用户图片管理插件 - 背景脚本
 * 支持用户登录认证和个人图库上传
 */

// 创建右键菜单
chrome.runtime.onInstalled.addListener(() => {
  // 竞品图库上传（保留原功能）
  chrome.contextMenus.create({
    id: 'upload-to-competitor',
    title: '上传到竞品图库',
    contexts: ['image']
  });

  // 个人图库上传（新功能）
  chrome.contextMenus.create({
    id: 'upload-to-my-library',
    title: '上传到我的图库',
    contexts: ['image']
  });
});

// 监听右键菜单点击
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'upload-to-competitor') {
    handleCompetitorUpload(info, tab);
  } else if (info.menuItemId === 'upload-to-my-library') {
    handleUserLibraryUpload(info, tab);
  }
});

/**
 * 处理竞品图库上传（原功能）
 */
async function handleCompetitorUpload(info, tab) {
  try {
    const config = await loadConfig();

    if (!config || !config.enabled) {
      showNotification('错误', '插件未启用');
      return;
    }

    if (!config.competitorUploadEndpoint) {
      showNotification('配置错误', '请先配置竞品上传接口');
      return;
    }

    const imageUrl = info.srcUrl;
    showNotification('上传中', '正在上传到竞品图库...');

    const imageBlob = await fetchImageAsBlob(imageUrl);

    // 调用原来的竞品上传接口
    await uploadToCompetitorLibrary(imageBlob, config, {
      originalUrl: imageUrl,
      pageUrl: tab.url,
      pageTitle: tab.title
    });

    showNotification('上传成功', '图片已上传到竞品图库');
  } catch (error) {
    console.error('上传失败:', error);
    showNotification('上传失败', error.message);
  }
}

/**
 * 处理个人图库上传（新功能）
 */
async function handleUserLibraryUpload(info, tab) {
  try {
    // 检查登录状态
    const { session, user } = await chrome.storage.local.get(['session', 'user']);

    if (!session || !user) {
      showNotification('未登录', '请先登录后再上传图片');
      return;
    }

    const config = await loadConfig();
    if (!config) {
      showNotification('配置错误', '请先配置插件');
      return;
    }

    const imageUrl = info.srcUrl;
    showNotification('上传中', '正在上传到我的图库...');

    const imageBlob = await fetchImageAsBlob(imageUrl);

    // 上传到用户个人图库
    await uploadToUserLibrary(imageBlob, config, user, {
      originalUrl: imageUrl,
      pageUrl: tab.url,
      pageTitle: tab.title
    });

    showNotification('上传成功', '图片已上传到我的图库');
  } catch (error) {
    console.error('上传失败:', error);
    showNotification('上传失败', error.message);
  }
}

/**
 * 加载配置文件
 */
async function loadConfig() {
  try {
    const response = await fetch(chrome.runtime.getURL('config.json'));
    return await response.json();
  } catch (error) {
    console.error('加载配置失败:', error);
    return null;
  }
}

/**
 * 获取图片为Blob
 */
async function fetchImageAsBlob(imageUrl) {
  try {
    const response = await fetch(imageUrl, {
      mode: 'cors',
      credentials: 'omit',
      cache: 'no-cache'
    });

    if (!response.ok) {
      throw new Error(`HTTP ${response.status}`);
    }

    const blob = await response.blob();

    if (!blob.type.startsWith('image/')) {
      throw new Error('不是有效的图片');
    }

    return blob;
  } catch (error) {
    throw new Error(`获取图片失败: ${error.message}`);
  }
}

/**
 * 上传到竞品图库
 */
async function uploadToCompetitorLibrary(imageBlob, config, metadata) {
  const formData = new FormData();

  const timestamp = Date.now();
  const extension = imageBlob.type.split('/')[1] || 'jpg';
  const filename = `competitor_${timestamp}.${extension}`;

  formData.append('file', imageBlob, filename);
  formData.append('metadata', JSON.stringify(metadata));
  formData.append('category', 'competitor');

  const headers = {};
  if (config.supabaseAnonKey) {
    headers['Authorization'] = `Bearer ${config.supabaseAnonKey}`;
    headers['apikey'] = config.supabaseAnonKey;
  }

  const response = await fetch(config.competitorUploadEndpoint, {
    method: 'POST',
    body: formData,
    headers: headers
  });

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.message || `上传失败: HTTP ${response.status}`);
  }

  return await response.json();
}

/**
 * 上传到用户个人图库
 */
async function uploadToUserLibrary(imageBlob, config, user, metadata) {
  // 检查文件类型
  const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/jpg'];
  if (!allowedTypes.includes(imageBlob.type)) {
    throw new Error('不支持的文件类型');
  }

  // 检查文件大小（10MB）
  if (imageBlob.size > 10 * 1024 * 1024) {
    throw new Error('文件大小超过10MB限制');
  }

  // 导入 Supabase 客户端
  const { createClient } = supabase;
  const supabaseClient = createClient(config.supabaseUrl, config.supabaseAnonKey);

  // 设置 session
  await supabaseClient.auth.setSession(await chrome.storage.local.get(['session']).then(r => r.session));

  // 检查配额
  const { data: profile, error: profileError } = await supabaseClient
    .from('user_profiles')
    .select('storage_quota_mb, storage_used_mb')
    .eq('id', user.id)
    .single();

  if (profileError) throw new Error('无法获取用户配置');

  const fileSizeMB = imageBlob.size / (1024 * 1024);
  if (profile.storage_used_mb + fileSizeMB > profile.storage_quota_mb) {
    throw new Error('存储空间不足');
  }

  // 生成唯一文件名
  const fileExt = imageBlob.type.split('/')[1];
  const fileName = `${user.id}/${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`;

  // 上传到 Storage
  const { error: uploadError } = await supabaseClient
    .storage
    .from('user-images')
    .upload(fileName, imageBlob);

  if (uploadError) throw uploadError;

  // 获取公共 URL
  const { data: { publicUrl } } = supabaseClient
    .storage
    .from('user-images')
    .getPublicUrl(fileName);

  // 提取文件名
  const originalFileName = metadata.originalUrl.split('/').pop().split('?')[0] || 'image';

  // 保存元数据到数据库
  const { error: dbError } = await supabaseClient
    .from('user_images')
    .insert({
      user_id: user.id,
      file_name: originalFileName,
      file_path: fileName,
      file_size_bytes: imageBlob.size,
      mime_type: imageBlob.type,
      image_url: publicUrl,
      thumbnail_url: publicUrl,
      storage_bucket: 'user-images',
      upload_source: 'browser-extension',
      metadata: {
        originalUrl: metadata.originalUrl,
        pageUrl: metadata.pageUrl,
        pageTitle: metadata.pageTitle
      }
    });

  if (dbError) throw dbError;

  return { success: true, url: publicUrl };
}

/**
 * 显示通知
 */
function showNotification(title, message) {
  chrome.notifications.create({
    type: 'basic',
    iconUrl: 'icons/icon128.png',
    title: title,
    message: message,
    priority: 2
  });
}

/**
 * 监听来自popup的消息
 */
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'uploadImage') {
    handleUserLibraryUpload({ srcUrl: request.imageUrl }, request.tabInfo)
      .then(() => sendResponse({ success: true }))
      .catch(error => sendResponse({ success: false, error: error.message }));
    return true;
  }
});
